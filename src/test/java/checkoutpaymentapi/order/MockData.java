...
class MockData {
	...
	static OrderData SubmitSuccessData() {
		return OrderData.builder().orderId(orderId).event(OrderEvent.orderCreated).build();
	}

	static OrderData paymentSuccessData() {
		return OrderData.builder().orderId(orderId).event(OrderEvent.paymentSuccess).build();
	}

	static OrderData paymentErrorData() {
		return OrderData.builder().event(OrderEvent.paymentError).orderId(orderId).build();
	}
}
